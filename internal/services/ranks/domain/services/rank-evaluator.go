...
func (r *RankEvaluator) Evaluate(db *gorm.DB, year int, quarter int) ([]*rank.Rank, error) {
	...
	if quarter != 0 {
		ranks, err := r.evaluateTopBuyQuarter(db, year, quarter)
		if err != nil {
			return nil, applicationError.Wrap(err)
		}
		result = append(result, ranks...)
		r.evaluateTopSellQuarter(db, year, quarter)
		r.evaluateTopHoldingQuarter(db, year, quarter)
	} else {
...
func (r *RankEvaluator) evaluateTopBuyQuarter(db *gorm.DB, year int, quarter int) (ranks []*rank.Rank, err error) {
...
func (r *RankEvaluator) evaluateTopSellQuarter(db *gorm.DB, year int, quarter int) {

}

func (r *RankEvaluator) evaluateTopHoldingQuarter(db *gorm.DB, year int, quarter int) {
